...
//  ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  ...
  // load and place image
   ...
   bg = loadImage("Domingo_arrieta2.jpg");
   bgSM = loadImage("Domingo_arrieta2 SM.jpg");
...
//    m1Y = height/2 - raggio;
  ...
  device = controll.getDevice(4);//gets the mouse on the LEFT
...
//println(colOut);
  ...
  pg1.beginDraw();
  pg1.image(bgSM, (-penXget * 10) + (pgwidth/2), (-penYget * 10) + (pgheight/2), imageX*10, imageY*10);
  pg1.strokeWeight(1);
  pg1.stroke(255, 0, 0);
  pg1.noFill();
  pg1.rect(pgwidth/2 - 2, pgheight/2 - 2, 4, 4);
  pg1.endDraw();
  image(pg1, pgXorigin, pgYorigin);
  
  pg2.beginDraw();
  pg2.image(bgSM, (-penXget * 2) + (pgwidth/2), (-penYget * 2) + (pgheight/2), imageX*2, imageY*2);
  pg1.strokeWeight(2);
  pg2.stroke(255, 0, 0);
  pg2.noFill();
  pg2.rect(pgwidth/2 - 3, pgheight/2 - 3, 6, 6);
  pg2.endDraw();
  image(pg2, pgXorigin, pgYorigin + pgheight); 
  
  pg3.beginDraw();
  pg3.image(bgSM, (-penXget / 2) + (pgwidth/2), (-penYget / 2) + (pgheight/2), imageX/2, imageY/2);
  pg1.strokeWeight(2);
  pg3.stroke(255, 0, 0);
  pg3.noFill();
  pg3.rect(pgwidth/2 - 5, pgheight/2 - 5, 10, 10);
  pg3.endDraw();
  image(pg3, pgXorigin, pgYorigin + (pgheight*2)); 
